...
def tensor2image(input_image, imtype=np.uint8, return_numpy=False, save_image_dir='None', tensor_normalized=True):
    ...
    if not isinstance(input_image, np.ndarray): # if the input_image is not a numpy array
        ...
        if image_numpy.shape[0] == 1:  # if this is a grayscale image, grayscale to RGB
...
class Image_Dataset(Dataset):
    ...
    def __init__(self, dataset_name, target_img_size, data_dir, input_grayscale=False):
        ...
        # print(self.input_image_fullnames)
        
    def __getitem__(self, index):
        if self.input_grayscale:
            image_pil = Image.open(join(self.input_dir, self.input_image_fullnames[index])).convert('L')
        else:
            image_pil = Image.open(join(self.input_dir, self.input_image_fullnames[index])).convert('RGB')
        # print(img.getbands(),img.size) # ('L',) ('R', 'G', 'B')
        
        img_tensor = img_transform_list(self.target_img_size, Normalize=True)(image_pil)
...
if __name__ == '__main__':
    ...
    dataset = Image_Dataset(dataset_name, target_image_size, dataset_direction)
    ...
    for index, item in enumerate(training_data_loader, 1): # 返回list中数据的索引和数据， 索引从1开始
        ...
        image_name = item[1]
        img_tensor = item[0].squeeze() # 将输入张量形状中的1 去除并返回。如果输入是形如(A×1×B×1×C×1×D)，那么输出形状就为： (A×B×C×D) 当给定dim时，那么挤压操作只在给定维度上。
        # print('index: %d, %s' % (index, image_name)) # item[1] is the file name here
        if index == 8:
            # print(image_name) # __getitem__ returns a tuple(元组)
            print(img_tensor.size())
            out_img = tensor2image(img_tensor, imtype=np.uint8, return_numpy=False, save_image_dir=output_direction+image_name[0], tensor_normalized=True)
            out_img.show()
